...
const TransactionTable = ({onViewDetails}: TCategoryTableProps) => {
  ...
  const getStatusColor = (status: string) => {
    switch(status.toLowerCase()) {
      ...
      case "paid":
        return "bg-green-100 tex-green-600 border-green-300"
      case "rejected":
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-gray-200">
            ...
            <th className="px-6 py-4 font-semibold">Costumer</th>
        ...
        <tbody>
          {transactionData.map((data, index) => (
            ...
            >
              ...
              <td className="px-6 py-4 font-medium">{data.costumer}</td>
              ...
              <td className="px-6 py-7.5 flex items-center gap-3 text-gray-600">
                <button onClick={onViewDetails} className="flex gap-2 cursor-pointer hover:bg-gray-100 w-fit py-1 px-2 rounded-md items-center">
                  <FiEye size={18} />
                  View Details
                </button>           
              </td>
